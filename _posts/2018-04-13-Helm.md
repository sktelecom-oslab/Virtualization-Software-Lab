...
이번 포스팅에서는 TACO를 이루는 오픈소스 기술 중  Helm에 대해서 자세히 이야기해 보도록 하겠습니다.

## Helm 이란?
 Helm은 deis라는 회사에서 시작한 Helm Classic프로젝트와 Google에서 진행되던 GCS Deployment Manager 프로젝트가 합해진 프로젝트입니다.
Helm Classic은 Kubernetes에 어플리케이션을 올리고 관리하는 툴을 만드는 프로젝트고 GCS Deployment Manager는 Kubernetes의 어머니격인 Google Cloud에 어플리케이션을 배포하고 관리하는 툴을 만드는 프로젝트였기 때문에 손을 잡을 수 있었던 것 같습니다. 그렇게 deis와 google의 협업으로 진행되던 helm 프로젝트는 CNCF의 한 프로젝트로 완전히 오픈소스화가 되었습니다.

 Helm은 Charts를 관리하며 이를 이용해서 어플리케이션을 Kubernetes 위에 배포하고 배포된 어플리케이션을 관리합니다.
